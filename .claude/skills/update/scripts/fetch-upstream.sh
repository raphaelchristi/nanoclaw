...
# Fetch upstream AOD Engine and extract to a temp directory.
...
if git remote get-url upstream &>/dev/null; then
  ...
  if echo "$ORIGIN_URL" | grep -q "raphaelchristi/aod-engine"; then
    REMOTE="origin"
  fi
fi

if [ -z "$REMOTE" ]; then
  echo "No upstream remote found. Adding upstream → https://github.com/raphaelchristi/aod-engine.git"
  git remote add upstream https://github.com/raphaelchristi/aod-engine.git
  REMOTE="upstream"
fi
...
TEMP_DIR=$(mktemp -d /tmp/aod-update-XXXX)
