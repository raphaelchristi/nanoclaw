---
name: update
description: "Update AOD Engine from upstream. Fetches latest changes, merges with your customizations and skills, runs migrations. Triggers on \"update\", \"pull upstream\", \"sync with upstream\", \"get latest changes\"."
---
...
```bash
test -d .aod && echo "INITIALIZED" || echo "NOT_INITIALIZED"
```

**If NOT_INITIALIZED:** Run `initSkillsSystem()` first:

```bash
npx tsx -e "import { initAodDir } from './skills-engine/init.js'; initAodDir();"
```
...
**No upstream remote:** The fetch script auto-adds `upstream` pointing to `https://github.com/raphaelchristi/aod-engine.git`. If the user forked from a different URL, they should set the remote manually: `git remote add upstream <url>`.
...
**Rollback:** If something goes wrong after applying but before cleanup, the backup is still in `.aod/backup/`. Run:
```bash
