...
export function createTempDir(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'aod-test-'));
}

export function setupAodDir(tmpDir: string): void {
  fs.mkdirSync(path.join(tmpDir, '.aod', 'base', 'src'), { recursive: true });
  fs.mkdirSync(path.join(tmpDir, '.aod', 'backup'), { recursive: true });
}
