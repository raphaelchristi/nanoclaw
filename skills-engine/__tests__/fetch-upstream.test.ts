...
describe('fetch-upstream.sh', () => {
  ...
  beforeEach(() => {
    ...
    upstreamBareDir = fs.mkdtempSync(
      path.join(os.tmpdir(), 'aod-upstream-'),
    );
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({ name: 'aod-engine', version: '2.0.0' }),
    );
    ...
    projectDir = fs.mkdtempSync(
      path.join(os.tmpdir(), 'aod-project-'),
    );
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({ name: 'aod-engine', version: '1.0.0' }),
    );
  ...
  it('uses existing upstream remote', () => {
    ...
    expect(status.TEMP_DIR).toMatch(/^\/tmp\/aod-update-/);
  ...
  it('uses origin when it points to raphaelchristi/aod-engine', () => {
    // Set origin to a URL containing raphaelchristi/aod-engine
    execSync(
      `git remote add origin https://github.com/raphaelchristi/aod-engine.git`,
  ...
  it('adds upstream remote when none exists', { timeout: 15_000 }, () => {
    ...
    expect(remotes).toContain('raphaelchristi/aod-engine');
  });
