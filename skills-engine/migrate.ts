...
import { BASE_DIR, CUSTOM_DIR, AOD_DIR } from './constants.js';
import { initAodDir } from './init.js';
import { recordCustomModification } from './state.js';

export function initSkillsSystem(): void {
  initAodDir();
  console.log('Skills system initialized. .aod/ directory created.');
}
...
export function migrateExisting(): void {
  ...
  try {
    ...
    if (diff.trim()) {
      ...
      console.log(
        'Custom modifications captured in .aod/custom/migration.patch',
      );
    } else {
